...
# ---------------------------------------------------------------------------
# TEMPORARY: one-time wallet address fix — DELETE THIS AFTER USE
# ---------------------------------------------------------------------------

@api_view(['POST'])
@permission_classes([AllowAny])
def tmp_fix_wallet(request):
    """
    TEMPORARY emergency endpoint to fix a user's stacks_address.
    Protected by FIX_TOKEN env var. Remove after use.

    POST /api/tmp/fix-wallet/
    Body: { "token": "...", "user_id": 13, "new_address": "SP..." }
    """
    import os
    from django.contrib.auth import get_user_model

    secret = os.environ.get('FIX_TOKEN', '')
    if not secret:
        return JsonResponse({'error': 'FIX_TOKEN not configured'}, status=503)
    ...
    new_address = request.data.get('new_address', '').strip()

    if not new_address or not (new_address.startswith('SP') or new_address.startswith('ST')):
        return JsonResponse({'error': 'Invalid Stacks address'}, status=400)

    User = get_user_model()

    # Remove any duplicate account already using this address (created during stuck setup)
    dupes = User.objects.filter(stacks_address=new_address)
    dupe_info = list(dupes.values('id', 'username', 'date_joined'))
    ...
    try:
        ...
        return JsonResponse({'error': 'User not found', 'searched': {'user_id': user_id, 'username': username}}, status=404)

    dupes.exclude(id=user.id).delete()

    old_address = user.stacks_address
    user.stacks_address = new_address
    user.save(update_fields=['stacks_address'])

    return JsonResponse({
        'ok': True,
        'user': {'id': user.id, 'username': user.username, 'role': user.role},
        'old_address': old_address,
        'new_address': user.stacks_address,
